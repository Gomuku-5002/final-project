...
#%%
# To find the position at which the game will end
# Return (999,999) if no such position exists.
...
#%%  
# To get a list of position which is not explored in the tree.
...
# To get a list of the adjacent position of stones in the chess board.
...
# To select a child of a node in the tree
...
# To assign a value to a leaf node.
        ...
        for (score, shape) in shape_score:
            if tmp_shap5 == shape or tmp_shap6 == shape:
                if score > max_score_shape[0]:
                    max_score_shape = (score, ((ff(m + (0+offset) * x_decrict), ff(n + (0+offset) * y_derice)),
                                               (ff(m + (1+offset) * x_decrict), ff(n + (1+offset) * y_derice)),
                                               (ff(m + (2+offset) * x_decrict), ff(n + (2+offset) * y_derice)),
                                               (ff(m + (3+offset) * x_decrict), ff(n + (3+offset) * y_derice)),
                                               (ff(m + (4+offset) * x_decrict), ff(n + (4+offset) * y_derice))), (x_decrict, y_derice))
...
# One run of MCST starting from the root node        
def run(board, node, ai_color):
...
def ai_move(board, color):
    ...
    timeout = time.time() + 4.95
    
    board_width = len(board)
    
    if sum(sum(board==0)) == board_width**2:            # if there is no stone in the chess board
         return (board_width//2,board_width//2)
    
    check = avoid_win(board,-color)                     # to check whether the opponent is gonna win
    if check == (999,999):
        check_ai = avoid_win(board,color)               # to check whether ai is gonna win
        if check_ai == (999,999):
            Node={
                'value': 0,
                'num': 0,
